        ...
        <div className="flex justify-between items-center py-3 mx-0">
          ...
          <div className="hidden lg:flex items-center space-x-8 flex-1 justify-center ml-6">
            {items.map(item => {
              ...
              return (
                <Link key={item.href} to={item.href} className={`font-medium transition-colors duration-200 relative group border-b-2 pb-1 text-sm ${isActive ? 'border-blue-600' : 'border-transparent'}`} style={{
                  color: '#0553A5'
                }}>
                  {item.label}
                  {!isActive && <span className="absolute -bottom-1 left-0 w-0 h-0.5 transition-all duration-200 group-hover:w-full" style={{
                ...
                <div className="flex flex-col space-y-6 mt-8">
                  ...
                  <div className="space-y-4">
                    {items.map(item => {
                          ...
                          <div key={item.label}>
                            ...
                            <div className="ml-4 space-y-2 mt-2">
                              {item.submenu.map(subItem => {
                                ...
                                return (
                                  <Link key={subItem.href} to={subItem.href} className={`block text-base py-1 transition-colors ${isSubActive ? 'font-medium' : 'opacity-80'}`} style={{
                                    color: '#0553A5'
                      ...
                      const isActive = isActiveLink(item.href);
                      return (
                        <Link key={item.href} to={item.href} className={`text-lg font-medium py-2 border-b border-border transition-colors ${isActive ? 'border-blue-600' : ''}`} style={{
                          color: '#0553A5'
                        }} onClick={() => setIsOpen(false)}>
                          {item.label}
                        </Link>
